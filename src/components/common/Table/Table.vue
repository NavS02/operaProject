  ...
  <table v-bind="{ ...$attrs }" :class="{ striped: striped, hover: hover }">
    <thead>
      <template v-for="field in _fields" :key="`thead-${field.key}`">
        <th
          ...
          @click="field.sortable && sortBy(field)"
        >
          ...
          <slot :name="`head(${field.key})`" :field="field" :data="this">{{
            field.label
          }}</slot>
        </th>
      </template>
    ...
    <tbody>
      <template
        ...
        :key="`trow-${item?.id ?? itemIndex}`"
      >
        <tr>
          <template
            v-for="field in _fields"
            :key="`tcell-${field.key + (item?.id ?? itemIndex)}`"
          >
            <td :class="{ [`tcell-${field.key}`]: true }">
              <slot
                :name="`cell(${field.key})`"
                :data="this"
                :item="item"
